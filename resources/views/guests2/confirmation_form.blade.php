        ...
        <div class="col-md-6">
            <div class="card shadow">
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('guest2.confirmation.store', $event->id) }}" enctype="multipart/form-data">
                        ...
                        <h4>Acompañantes (Máximo: {{ $event->limit_partners }})</h4>
                            <div id="partners-container">
                                @for($i = 0; $i < min($event->limit_partners, old('partners', 1)); $i++)
                                    <div class="partner-form mb-3">
                                        <label>Nombre del Acompañante {{ $i + 1 }}</label>
                                        <input type="text" class="form-control" 
                                            name="partners[{{ $i }}][name]" 
                                            value="{{ old('partners.'.$i.'.name') }}" required>
                                    </div>
                                @endfor
                            </div>
                        
                                @if($event->limit_partners > 1)
                                    <button type="button" id="add-partner" class="btn btn-success mb-3">
                                        Agregar Acompañante
                                    </button>
                                @endif
...
<script>
    document.getElementById('add-partner').addEventListener('click', function() {
        ...
        if (count < {{ $event->limit_partners }}) {
            ...
            alert('Llegaste al límite máximo de acompañantes');
        }
    });
</script>
